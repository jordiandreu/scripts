...
{
    NI_DEV='None'
    DEV=$(/sbin/lspci | grep -i 'National')
    if [ "$DEV" != '' ]; then
        NI_DEV=$DEV
    fi
    echo "$NI_DEV"
...
{
    ROCKETPORT_DEV='None'
    DEV=$(/sbin/lspci | grep -i 'Rocketport')
    if [ "$DEV" != '' ]; then
        ROCKETPORT_DEV=$DEV
    fi
    echo "$ROCKETPORT_DEV"
...
# Populating the stdout pipe for redmine:
echo "
h1. $TITLE
...
$(section_h2_rmf "National Instruments Card(s)" get_ni)
$(section_h2_rmf "AdLink Card(s)" get_adlink)
$(section_h2_rmf "Rocketport Card(s)" get_rocketport)
...
# Remove this exit command if you wan to generate the output as html or redmine formatted files.
exit
...
cat > $HTML_FILENAME <<- _EOF_
    ...
    $(section_h2_html "National Instruments Card(s)" get_ni)
    $(section_h2_html "AdLink Card(s)" get_adlink)
    $(section_h2_html "Rocketport Card(s)" get_rocketport)
